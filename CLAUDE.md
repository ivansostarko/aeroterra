...
**Declared exception — AT-H12 Griffin's model.** `Assets/Resources/Models/AT-H12/drone.fbx` is a hand-modeled, user-supplied binary FBX, imported at the user's explicit request as the twelfth drone. It has been re-supplied/re-exported once since the original import — current file is an MQ-9-style fixed-wing airframe (single rear pusher `Propeller`/`Blades`, `Aileron`/`Elevator`/`Rudder`/`Flap` control surfaces, `Wheel`/`Strut`/`Scissor` landing gear, `HellfirePylonL`/`R` weapon stations, nose `Sensor`/`SensorMount` turret) — `Class = VtolCargo` and the 4-rotor VTOL description/stats were deliberately left as a hovering cargo quad, but `PayloadKind` was later changed to `GuidedAmmunition` (matching Kestrel's "Guided munitions (simulated)" wording) at the user's explicit request, so Griffin's Workshop LOADOUT ▸ AMMUNITION PAYLOAD tab now reads as guided-munitions even though everything else about the spec still describes a cargo VTOL — an intentional mismatch, same spirit as the model-vs-spec mismatch already documented here, not a bug to "fix" back to Cargo. `DroneSpecification.ModelKind.ImportedMesh` routes it through `Assets/Scripts/Procedural/ImportedDroneBuilder.cs`, which `Resources.Load<GameObject>("Models/AT-H12/drone")`s and instantiates it — same `Resources.Load` pattern as the Fire VFX prefabs above — instead of building primitives at runtime like every other `*Builder.cs`. `Assets/Editor/ImportedModelPostprocessor.cs` (an `AssetPostprocessor.OnPreprocessModel`, scoped to `Assets/Resources/Models/`) configures its import settings automatically so nobody has to hand-tune it in the Inspector. Because the real-world size of a hand-modeled mesh isn't known ahead of time the way a procedural builder's own coordinates are, `ImportedDroneBuilder` measures the instantiated model's actual renderer bounds and caches it (`LastMeasuredWingspanM`) for `DroneFactory.ReferenceWingspanM` to scale against, rather than using a hardcoded constant like every other `DroneModelKind`. Rotor spin is best-effort: any child object named containing "rotor"/"prop"/"blade" gets a `RotorSpinner` attached automatically (matches this FBX's `Propeller`/`Blades` fine, no rename needed). Since there's still no Editor available to inspect the imported hierarchy or verify the final post-import local axes, `ImportedDroneBuilder.SpinRotors` doesn't assume a rotor layout or spin axis: matches are clustered by their actual mesh-geometry proximity (so a hub+blades pair mounted at the same spot shares one spin direction instead of alternating like genuinely separate rotors would) and each spins around whichever of its own local axes its mesh bounds are thinnest along (a flat multirotor disc spins around up same as before; a nose/tail-mounted pusher prop spins around forward instead) — verify both in the Editor once one's available and hardcode/adjust if the heuristic guessed wrong. The same applies to payload drop: `PayloadDropper` looks for a `"PayloadVisual"` child (optionally with `"Store*"` grandchildren) the way every other cargo/military drone has — this FBX has no such child (nor a cargo bay at all, being a fixed-wing recon airframe), so the drop action and Workshop payload-visibility toggle silently no-op regardless of `PayloadKind`. Don't take this as license to import further 3D models without asking first — it's specific to Griffin.

**Drone skins are runtime-generated textures, not imported images.** `Assets/Scripts/Procedural/DroneSkinBuilder.cs` paints camo/stripes/split-fade/digital patterns into a `Texture2D` at runtime from a body/accent color pair — same "100% procedural" property as everything else, just with a generated texture instead of a flat `Material.color`. Accent color is always the spec's fixed `DefaultAccentColor`. Body color is that same spec default UNLESS the Workshop's LOADOUT ▸ MAIN COLOR picker set one: `CustomDroneData.HasCustomBodyColor` gates `BodyR/G/B` (false — the default, and what every pre-existing save deserializes to — means "use the spec default", so old saves don't change appearance); `WorkshopController.SetBodyColor()`/`CurrentBodyColor` read/write it, `DroneFactory.Spawn` is where it actually gets applied. `CustomDroneData.AccentR/G/B` remain legacy-only, kept just so old saves still deserialize.

**Two onboard power systems.** Most drones use `BatterySystem` (Wh capacity); AT-L3 Locust, AT-J9 Wraith and AT-U11 Bison use `FuelSystem` (liters) instead — see `DroneSpecification.PowerSystem` (`PowerSystemType.Battery`/`.Fuel`) and the shared `IPowerSource` interface `DroneFlightController` talks to via its private `_power` field (resolved in `Start()`, not `Awake()` — `Spec` isn't assigned until after `AddComponent<DroneFlightController>()` returns). A new fuel-powered drone needs `PowerSystem = PowerSystemType.Fuel` and a populated `FuelOptionsL` array in its `ProjectBootstrap.cs` spec block; `DroneFactory.Spawn` reads `spec.PowerSystem` to decide which component to attach. `BatterySystem.PerformanceFactor(temperatureC)` (a static method, fed by `SettingsData.TemperatureC` from Settings ▸ Flying Conditions) derates thrust ceiling outside a 5–35°C comfort band — applied in `DroneFlightController.FixedUpdate`'s `_batteryPerfFactor`, multiplied into both `TickMultirotor`/`TickFixedWing`'s thrust clamp, but only when `PowerSystem == Battery`; `FuelSystem` has no equivalent.

**Military payload types.** `DroneSpecification.PayloadKind` (`Cargo`/`Warhead`/`GuidedAmmunition`/`DropAmmunition`) is a real behavioral field now, not just the old decorative `PayloadTypeName` string — it drives `PayloadModelBuilder`'s per-type procedural munition mesh, `DroppedPayloadAerodynamics`' per-type fall/stabilization tuning, and pitched drop/impact audio (`AudioManager.PlayBombDrop/PlayBombExplosion(pos, pitch)` overloads), all wired up in `PayloadDropper.cs`. Behaviorally live (actually drives a `PayloadDropper`-driven multi-store mount) for Hornet=DropAmmunition, Kestrel=GuidedAmmunition, Bison=Warhead, and Griffin=GuidedAmmunition (Griffin's `PayloadDropper` no-ops regardless — see the Griffin exception above — so this only affects its Workshop badge/label, not an actual drop), plus the two cargo-pod drones (Pelican/Osprey=Cargo, whose existing hand-detailed pod models are deliberately never model-swapped). Kamikaze-class drones (Vespid/Locust/Wraith) have no `PayloadDropper` at all — their warhead is integral, see `IsKamikazeClass` — but all three now also carry `PayloadKind = Warhead` explicitly (previously left unset/defaulting to `Cargo`, which showed the wrong crate glyph on their Workshop LOADOUT ▸ AMMUNITION PAYLOAD badge); this is cosmetic-only for them since nothing ever reads it in flight.

**AT-R4 Hornet: two selectable payload categories, live-switchable in flight.** Unlike every other drone (one fixed `PayloadKind`), Hornet's `DroneSpecification.AvailablePayloadKinds = { DropAmmunition, Warhead }` — an empty array on every other spec (meaning "just use `PayloadKind`, no picker") but 2 entries here at the user's explicit request. WorkshopUI's LOADOUT ▸ AMMUNITION PAYLOAD tab detects `AvailablePayloadKinds.Length > 1` and swaps in a category-picker row (`BuildPayloadKindCards`) above the weight `OptionRow` instead of the normal fixed single-badge layout (`BuildPayloadRow`) every other drone still uses unchanged. The pick is saved to `CustomDroneData.SelectedPayloadKind`/`HasSelectedPayloadKind` (same `HasCustomBodyColor`-style gating — unset/old saves fall back to `spec.PayloadKind`, i.e. `DropAmmunition`) and resolved once by `DroneFactory.Spawn` into `DroneFlightController.EffectivePayloadKind`, which `PayloadDropper`/`FlightHUD` read instead of `Spec.PayloadKind` directly.

`InputManager.PayloadSwitchAction` (**J**, gamepad left bumper) makes this same pick live-switchable mid-flight, not just a pre-flight Workshop choice — `PayloadDropper.TrySwitchPayloadKind()` cycles `AvailablePayloadKinds` and reassigns `EffectivePayloadKind` directly (a no-op on every drone with fewer than 2 entries, i.e. everyone else), re-skinning the store models immediately via the same `PayloadModelBuilder.Rebuild` call `Start()` uses at spawn. **Switching to Warhead redefines what `[I]` does, Hornet-only** (gated on `Spec.Id == "at-r4"`, so AT-U11 Bison's own fixed `Warhead` PayloadKind is completely unaffected and still just drops a warhead-shaped bomb): `PayloadDropper.TryDrop()` calls `DroneFlightController.DetonateAtCurrentPosition()` instead of releasing a store — the same full fire/explosion/audio/respawn sequence a real kamikaze collision uses (`Detonate`), just voluntarily triggered instead of impact-triggered. `FlightHUD`'s drop-hint/payload-label text also branches on this (`hornetSelfDestruct`) so it never shows the normal "[I] DROP" wording while this mode is armed. If a second drone ever needs multiple live-switchable categories, this is the pattern to reuse — but the self-destruct-on-Warhead behavior itself is Hornet-specific, not implied by `AvailablePayloadKinds.Length > 1` alone.

**Additional loadout slots.** Smoke Screen (a real always-on trailing particle effect, `DroneFactory.BuildSmokeScreen`), Horn (a real one-shot warning-horn sound, H key, `DroneHornController`, `Assets/Resources/Audio/sfx/drone/horn.mp3`), and Comms (`AeroTerra.Drone.CommsType` — Radio/5G/Analog-Wire) all add real weight via `DroneFlightController.ExtraLoadoutMassKg`, set once by `DroneFactory.Spawn` from `CustomDroneData`. Comms drives no jamming/interference mechanic (none exists) — it's descriptive/flavor only (see `LoadoutExtras.cs`). Horn is enabled by default both for a fresh Workshop config (`WorkshopController.Show`) and for a stock Free Flight spawn with no Workshop customization at all (`custom == null` — see `DroneFactory.Spawn`'s `hornEquipped`), same "manufacturer default loadout" precedent Smoke Screen/Parachute already established.

**Font**: no custom `TMP_FontAsset` has ever been imported — every `TextMeshProUGUI` in the game goes through `UIBuilder.Label()`, which auto-loads `Resources/Fonts/AeroTerraFont.asset` if present and falls back to TMP's default otherwise. To change the game's typeface: import a `.ttf`/`.otf`, generate a Font Asset via the Editor (Window ▸ TextMeshPro ▸ Font Asset Creator — this cannot be done headlessly), and save it at that exact Resources path. No other code changes needed. `docs/08-FONTS.md` tracks every font actually in the project and where it's used — **keep it up to date any time a font is added, swapped, or removed**, the same turn you make that change.

**Free Flight dev/cheat console**: press `~` (backtick) in flight to drop a Quake/Source-style command console over the HUD (`Assets/Scripts/UI/GameConsoleUI.cs`) — doesn't pause the game, just disables the real gameplay `InputAction`s while it has typing focus (`InputManager.SetGameplayInputEnabled`) so command text doesn't also trigger hotkeys. Commands are a plain name → handler dictionary (`GameConsoleUI.RegisterCommands`); the first one, `speed <km/h>`, overrides `DroneFlightController.EffectiveMaxSpeedKmh` (floor-clamped to 0). `docs/10-CHEATS.md` tracks every cheat command that exists — **keep it up to date any time a cheat is added, changed, or removed**, the same turn you make that change; use the `/add-cheat` skill (`.claude/commands/add-cheat.md`) to do both halves of that in one pass.
...
| Dev/cheat console | `Assets/Scripts/UI/GameConsoleUI.cs` (see `docs/10-CHEATS.md`) |
...
- **Unity version**: 6000.0.80f1 (see `ProjectSettings/ProjectVersion.txt`) — Unity 6, not 2022.3 LTS. Every physics site in the codebase already uses the Unity 6 `Rigidbody` API names (`linearVelocity`, `linearDamping`, `angularDamping`), not the pre-6.0 `velocity`/`drag`/`angularDrag` names — match that when adding new physics code.
...
- **New cheats**: add a handler method to `GameConsoleUI` and register it in `RegisterCommands()` (one dictionary entry), wire whatever it affects via a public setter/override on the relevant system (see `DroneFlightController.SetMaxSpeedOverride` for the pattern), then add a row to `docs/10-CHEATS.md` — use the `/add-cheat` skill to do this whole flow in one pass so the doc never falls out of sync.
...
- **`Skybox/Procedural` shader stripping**: `SkySystem.ApplyProceduralSkybox` creates its skybox material from `Shader.Find("Skybox/Procedural")` at runtime — nothing in the project references it at edit time, so a build's shader stripping could omit it. It gracefully no-ops if so (falls back to the flat `Camera.backgroundColor` fill that existed before), but the sky preset won't visually change the sky dome itself in that case — add `Skybox/Procedural` to Project Settings ▸ Graphics ▸ Always Included Shaders if a build ever shows this.
- **Free Flight's spawn-location map preview needs internet**: `FreeFlightMenuUI`'s dark-tiled map background (`LoadDarkMapTile`) fetches a real map tile from CARTO's keyless "dark_all" basemap over HTTP. It fails silently (falls back to the synthetic origin/distance plot that existed before) with no connection — same "never assume a network call succeeds" convention as every `Resources.Load` in this codebase — but unlike the Cesium ion dependency, this one has no token/config to set; it just needs outbound HTTPS access to `basemaps.cartocdn.com`.
...
- `docs/08-FONTS.md` — every font actually shipping in the project, where it's used, how to swap it — keep current
- `docs/09-FLIGHT-MODELS.md` — flight-model specification: review of the shipped physics plus the full definition of all four models (multirotor / fixed-wing / VTOL hybrid / rocket) — controls, aerodynamics, envelopes, failure modes, phased implementation plan. Part I is a review of current code; Parts II–VI are the target design and are **not implemented yet**
- `docs/10-CHEATS.md` — every Free Flight dev/cheat console command, what it does, and how to add a new one — keep current any time a cheat is added, changed, or removed
