...
**AT-R4 Hornet: two selectable payload categories.** Unlike every other drone (one fixed `PayloadKind`), Hornet's `DroneSpecification.AvailablePayloadKinds = { DropAmmunition, Warhead }` — an empty array on every other spec (meaning "just use `PayloadKind`, no picker") but 2 entries here at the user's explicit request. WorkshopUI's LOADOUT ▸ AMMUNITION PAYLOAD tab detects `AvailablePayloadKinds.Length > 1` and swaps in a category-picker row (`BuildPayloadKindCards`) above the weight `OptionRow` instead of the normal fixed single-badge layout (`BuildPayloadRow`) every other drone still uses unchanged. The pick is saved to `CustomDroneData.SelectedPayloadKind`/`HasSelectedPayloadKind` (same `HasCustomBodyColor`-style gating — unset/old saves fall back to `spec.PayloadKind`, i.e. `DropAmmunition`) and resolved once by `DroneFactory.Spawn` into `DroneFlightController.EffectivePayloadKind`, which `PayloadDropper`/`FlightHUD` now read instead of `Spec.PayloadKind` directly — so the rest of the flight-time payload pipeline (munition mesh, drop/impact audio pitch, HUD pips) needed no structural changes, just a swapped read. If a second drone ever needs multiple categories, this is the pattern to reuse.
